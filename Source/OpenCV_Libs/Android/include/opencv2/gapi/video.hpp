...
struct BackgroundSubtractorParams
{
    ...
    /** Full constructor
     ...
     thrshld For MOG2: Threshold on the squared Mahalanobis distance between
    ...
    whether a pixel is close to that sample.
     detect If true, the algorithm will detect shadows and mark them. It decreases the
    speed a bit, so if you do not need this feature, set the parameter to false.
     lRate The value between 0 and 1 that indicates how fast the background model is learnt.
...
@note Function textual ID is "org.opencv.video.buildOpticalFlowPyramid"
 ...
 winSize            window size of optical flow algorithm. Must be not less than winSize
...
@note Function textual ID is "org.opencv.video.calcOpticalFlowPyrLK"
 ...
 prevImg first 8-bit input image (GMat) or pyramid (GArray<GMat>) constructed by
buildOpticalFlowPyramid.
 nextImg second input image (GMat) or pyramid (GArray<GMat>) of the same size and the same
type as prevImg.
 prevPts GArray of 2D points for which the flow needs to be found; point coordinates must be
single-precision floating-point numbers.
 predPts GArray of 2D points initial for the flow search; make sense only when
...
moves by less than criteria.epsilon).
 ...
 -   **OPTFLOW_LK_GET_MIN_EIGENVALS** use minimum eigen values as an error measure (see
     ...
     error measure.
 minEigThresh the algorithm calculates the minimum eigen value of a 2x2 normal matrix of
...
@note Functional textual ID is "org.opencv.video.KalmanFilter"
 ...
 haveMeasurement dynamic input flag that indicates whether we get measurements
at a particular iteration .
 control input matrix: 32-bit or 64-bit float 1-channel matrix contains control data
